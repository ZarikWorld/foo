...
{
    public class MainViewModel_old : INotifyPropertyChanged
    {
        #region Fields
        ...
        public ObservableCollection<JxTask> backlogTasks
        {
            ...
            set
            {
                if (value != _backlogTasks)
                {
                    if (_backlogTasks != null)
                    {
                        _backlogTasks.CollectionChanged -= OnTasks_CollectionChanged;
                    }
        ...
        public MainViewModel_old()
        {
            try 
            {
                Log.Information("Enter {MethodName} method.", nameof(MainViewModel_old));
                this.getMitarbeiters();            
         
                _tasks = new ObservableCollection<JxTask>();
                _tasks.CollectionChanged += OnTasks_CollectionChanged;

                _backlogTasks = new ObservableCollection<JxTask>();
                _backlogTasks.CollectionChanged += OnTasks_CollectionChanged;
                ...
                Log.Information("Exit {MethodName} method.", nameof(MainViewModel_old));
            }
            catch (Exception ex)
            {
        ...
        public bool getProjectsIssues()
        {
            try
            {
                ...
                //backlogTasks.Clear();
                //if (currentSelectedMitarbeiter != 0)
                //{
                //    foreach (var taskX in tasks)
                //    {
                //        if (taskX.mitarbeiter_id != currentSelectedMitarbeiter)
                //        {
                //            continue;
                //        }
                //        backlogTasks.Add(taskX);
                //    }
                //} else
                //{
                //    foreach (var taskX in tasks)
        ...
        public void updateTodosCollection(bool hideImTestIssues = true)
        {
            Log.Information("Enter {MethodName} method.", nameof(updateTodosCollection));
            var dropDownUserTasks = (from x in tasks
                                     where x.mitarbeiter_id == todoDropDownUser &&
                                           x.sortOrder.HasValue &&
                                           x.sortOrder.Value > 0 &&
                                           (!hideImTestIssues || x.status != enumStatus.imTest)
                                     select x).ToList();

            Todos?.Clear();
            foreach (var task in dropDownUserTasks)
            {
                this.Todos?.Add(Todo.JxTask2Todo(task, mitarbeiters));
            }
            Log.Information("Exit {MethodName} method.", nameof(updateTodosCollection));
        }
